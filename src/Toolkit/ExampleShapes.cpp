...
)
{
  ...
  //MODIFICACION: Antonio Lopez Jaimes
  //La D es ahora más grande
  return FrameworkFunctions::calculate_f( 8.0, x, h, S );
//  return FrameworkFunctions::calculate_f( 1.0, x, h, S );
}
...
vector< double > Shapes::WFG1_shape( const vector< double >& t_p, const vector<double>& S  )
{
  assert( vector_in_01( t_p ) );
  assert( t_p.size() >= 2 );
...
vector< double > Shapes::WFG2_shape( const vector< double >& t_p, const vector<double>& S )
{
  assert( vector_in_01( t_p ) );
  assert( t_p.size() >= 2 );
...
vector< double > Shapes::WFG3_shape( const vector< double >& t_p, const vector<double>& S )
{
  assert( vector_in_01( t_p ) );
  assert( t_p.size() >= 2 );
...
vector< double > Shapes::WFG4_shape( const vector< double >& t_p, const std::vector<double>& S )
{
  assert( vector_in_01( t_p ) );
  assert( t_p.size() >= 2 );
